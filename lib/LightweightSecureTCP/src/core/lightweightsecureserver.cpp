...
LightweightSecureServer::LightweightSecureServer(int port)
    ...
    , m_serverSocket(LWSSocket(m_keepAliveIntervalMs ,m_inactivityTimeoutMs))
    , m_port(port)
    , m_running(false)
{
...
void LightweightSecureServer::setKeepAliveConfig(int keepAliveIntervalMs, int inactivityTimeoutMs)
{
    ...
    m_serverSocket.setKeepAliveConfig(keepAliveIntervalMs,inactivityTimeoutMs);
}
